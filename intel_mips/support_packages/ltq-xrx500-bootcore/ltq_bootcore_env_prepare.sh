...
fix_config_options() {
	...
	echo "CONFIG_TARGET_NAME=\"mips-openwrt-linux\"" >> $BASEDIR/.config
	echo "CONFIG_TOOLCHAIN_PREFIX=\"mips-openwrt-linux-\"" >> $BASEDIR/.config

	# by default sstrip is not activated for external tool chains.
	echo "CONFIG_USE_SSTRIP=y" >> $BASEDIR/.config
